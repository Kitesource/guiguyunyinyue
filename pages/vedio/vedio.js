...
Page({
  ...
  //点击切换到导航的回调
  changeNav(event) {
    //通过id向event传参时，如果穿的是number会自动转换成string,通过data-id的形式则不会转换
  ...
  handleTimeUpdate(event) {
    ...
    /* 
      思路： 判断记录播放时长的videoUpdateTime数组中是否有当前视频的播放记录
        1.如果有，在原有的播放记录中修改当前的播放时间为当前播放时间
        2.如果没有，需要在数组中添加当前的播放时间
    */
